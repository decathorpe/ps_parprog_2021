                  ...
                  double c[4], int k)
{
  ...
  // parallelizing outermost loop is valid (with private r1, r2):
  // - r and c are only ever read from and don't have dependences
  // - r1 and r2 are shared partial results that depend on i3 and i2,
  //   and need to be private to each thread (small memory allocation overhead)
  // - true dependence on u[i3][i2][i1] is harmless since memory
  //   access is specific to each iteration and never shared
                  ...
                  double a[4], int k)
{
  ...
  // parallelizing outermost loop is valid (with private u1, u2):
  // - u and a are only ever read from and don't have dependences
  // - u1 and u2 are shared partial results that depend on i3 (and i2),
  //   and need to be private to each thread (small memory allocation overhead)
  // - no true dependence in second innermost loop (unlike psinv),
  //   since r != v in this case
                  ...
                  void *os, int m1j, int m2j, int m3j, int k)
{
  ...
  // parallelizing outermost loop is valid (with private x1, y1, x2, y2):
  // - r is only ever read from and doesn't have dependences
  // - y1 and y1 are shared partial results that depend on i3 (and i2),
  //   and need to be private to each thread (small memory allocation overhead)
  // - no true dependence in second innermost loop (unlike psinv),
  //   since r != s in this case
                   ...
                   void *ou, int n1, int n2, int n3, int k)
{
  ...
  if (n1 != 3 && n2 != 3 && n3 != 3) {
    // parallelizing outermost loop is valid (with private z1, z2, z3):
    // - z is only ever read from and doesn't have dependences
    // - z1, z2, z3 are shared partial results that depend on i3 (and i2),
    //   and need to be private to each thread (small allocation overhead)
    // - true dependence between all u[][][] = u[][][] somethings,
    //   but the memory location is distinct in each iteration
    ...
    // this else branch does not seem to be executed often (or at all),
    // it does not even show up in profiling data. no need to parallelize
    if (n1 == 3) {
      d1 = 2;
      t1 = 1;
    } else {
